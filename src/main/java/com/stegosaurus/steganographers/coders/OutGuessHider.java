...
public class OutGuessHider extends OutGuess {
  /**
   * The actual hider, does the heavy lifting of embedding data in the carrier.
   */
  private class Hider {
    /**
     * The cover image.
     ...
     * @param cover the cover image.
     */
    public Hider(int[] cover, TIntIntMap freq, TIntDoubleMap tolerances) {
   ...
   */
  public OutGuessHider(String key) {
    super(key);
  }
  ...
  public void hide(int[] cover, byte[] message) {
    ...
    Hider hider = new Hider(cover, TCollections.unmodifiableMap(freq),
      tolerances);
    int[] result = hider.hide(message);
